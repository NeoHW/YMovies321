...
function fetchImages(movieId: string) {
    ...
    return fetch(`https://api.themoviedb.org/3/movie/${movieId}/images`, options)
        .then(response => {
            return response.json();
        });
}

export default function PicturesScrollBar(movieId: string | null) {
    const [images, setImages] = useState(null);
    // console.log(movieId); why is this an object?
    // console.log(movieId.movieId);
    useEffect(() => {
        fetchImages(movieId.movieId).then((data) => {
