...
export default function Score({ firebaseData }: { firebaseData: any }) {
    ...
    return (
        <div>
            <Typography align="center" sx={{ color: "#d8dbda" }} variant="subtitle1" >
                <p>{firebaseData.vote_average.toFixed(1)} / 10, {firebaseData.vote_count} votes</p>
            </Typography>
            <Rating
                value={value}
                max={10}
                onChange={(event, newValue) => {
                    setValue(newValue);
                    playerWriteNewMovieScore(user, firebaseData.id, newValue)
                    movieWriteNewScore(user, firebaseData.id, newValue);
                }}
            />
